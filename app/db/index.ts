...
const baseDir = moduleDir.startsWith('/$bunfs') ? process.cwd() : resolve(moduleDir, '../..')

const rawDbPath = process.env.DB_PATH || 'data/bitk.db'
const dbPath = rawDbPath.startsWith('/') ? rawDbPath : resolve(baseDir, rawDbPath)
...
if (existsSync(journalPath)) {
  // Filesystem migrations available (dev / non-compiled mode)
